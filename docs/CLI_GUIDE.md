...
[README](../README.md) · [Recording demo](DEMO_GUIDE.md) · [Architecture](ARCHITECTURE.md)

Run source examples from the repository root. This reference retains the detailed command contracts; the README and demo guide provide shorter, repeatable PowerShell walkthroughs.
...
This guarantees **at-most-one canonical workflow per event fingerprint within a shared local SQLite state store**. Independent state directories have independent identities. Network filesystems, cross-machine leases and authenticated event producers are unsupported. Ingestion and scanning are one-shot operations; the separate worker CLI can poll queued jobs.
...
- The project has no web service, account system, time-based scheduler, Excel parser, automatic source-data repair, or autonomous business actions. The planner uses a fixed DAG; it does not ask a model to invent a workflow.
